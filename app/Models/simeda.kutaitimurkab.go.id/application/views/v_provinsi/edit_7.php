...
$cek_provinsi = $this->mskegiatan_model->get_cek_provinsi($data->id_ms_keg);
$approved = $cek_provinsi && $cek_provinsi->updated_on>0;
?>
<form method="post">
...
<form method="post" action="<?=base_url('v_provinsi/submit');?>">
...
<blockquote class="box box-success">
    ...
    <div class="box-footer">
  	    <div>
  	    	" Metadata Kegiatan, Metadata Variabel, dan Metadata Indikator dalam kuesioner ini telah melalui proses pemeriksaan oleh <b>BPS Provinsi</b> dan siap untuk dilanjutkan ke proses berikutnya. "
  	    </div>
        <div class="checkbox pull-left" style="margin-top:0; margin-bottom:0">
          <label>
            <input type="checkbox" name="cek_keg" style="height: 24px" <?php if($cek_provinsi && $cek_provinsi->cek_keg) echo 'checked';?>> Approval MS-Kegiatan
          </label> <br>
          <label>
            <input type="checkbox" style="height: 24px" <?php if($cek_provinsi && $cek_provinsi->cek_var) echo 'checked';?>> <?=anchor('v_provinsi/variabel/'.$data->id_ms_keg,'Approval MS-Variabel','target=_variabel');?>
          </label> <br>
          <label>
            <input type="checkbox" style="height: 24px" <?php if($cek_provinsi && $cek_provinsi->cek_ind) echo 'checked';?>> <?=anchor('v_provinsi/indikator/'.$data->id_ms_keg,'Approval MS-Indikator','target=_indikator');?>
          </label> <br>
        </div>
