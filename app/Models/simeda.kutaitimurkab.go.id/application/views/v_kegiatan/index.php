...
?>
	<?php foreach($this->mskegiatan_model->all_by_wilayah($this->session->userdata('id_wilayah')) as $row){
		...
		$cek = $cek_provinsi? $cek_provinsi->cek_keg + $cek_provinsi->cek_var + $cek_provinsi->cek_ind : 0;
		...
		echo '<blockquote key="'.$row->id_ms_keg.'" '.($cek_provinsi && $cek_provinsi->cek_keg?'cek=1':($cek_provinsi && !$cek_provinsi->cek_keg && $cek_provinsi->catatan? 'cek=2': '')).'><judul>'.$row->judul_kegiatan.' ('.$row->tahun.')</judul>'.
...
$('[key]').each(function(){
	...
	$(this).find('judul').html('<a href="<?=base_url('v_kegiatan/edit/');?>'+key+'">'+$(this).find('judul').html()+'</a>');
	$(this).find('user').html('<a href="<?=base_url('v_kegiatan/instansi/');?>'+userid+'">'+$(this).find('user').text()+'</a>');
	$(this).find('var').html('<a href="<?=base_url('v_variabel/edit/');?>'+key+'">'+$(this).find('var').text()+'</a>');
	$(this).find('ind').html('<a href="<?=base_url('v_indikator/edit/');?>'+key+'">'+$(this).find('ind').text()+'</a>');
	...
	$('.label.bg-gray').attr('title','Menunggu approval');
...
$('span.prov').css('cursor','pointer').click(function(){
	...
	$.get('<?=base_url('ms_kegiatan/get_cek_provinsi/');?>'+id_ms_keg, function(data){
		console.log(data);
		if(data.result==1){
			$('#prov-dialog').modal('show');
			$('#cek_keg').attr('class','fa fa'+ (data.cek_kegiatan==1?'-check':'') +'-square-o');
			$('#cek_var').attr('class','fa fa'+ (data.cek_variabel==1?'-check':'') +'-square-o');
			$('#cek_ind').attr('class','fa fa'+ (data.cek_indikator==1?'-check':'') +'-square-o');
			$('#cek_catatan').html(data.catatan);
			$('#cek_pemeriksa').html(data.pemeriksa);
